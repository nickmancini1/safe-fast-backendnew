...
- QQQ CFB option context remains recorded as `unknown` because evidence files were not changed. The narrow listing-aware exception is now selector-implemented and regression-tested as `caution`, but evidence fill, headline context, execution context, and complete-caution promotion remain blocked.
