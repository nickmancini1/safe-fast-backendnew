...
- Execution-context rule status: first conservative rule accepted and implemented for regression work only. Quote age `<= 60` seconds is `clean`, quote age `> 60` seconds and `<= 5` minutes is `caution`, quote age `> 5` minutes or quote/spread/size/volume/no-fallback failure is `fail`, and missing source/rule proof is `unknown`. Data-only execution-context regression fixtures exist at `historical_signal_replay/fixtures/qqq_cfb_execution_context_regression_fixtures.json` with `13` cases, and `historical_signal_replay/execution_context_calculator.py` passes them through `tests/test_execution_context_calculator.py`. Later long-call testing uses ask price only as the fill basis. The target quote at `2026-04-13T16:06:30.640301037Z` is about `23` minutes `29` seconds old at setup, so the tested target execution status is `fail`.
