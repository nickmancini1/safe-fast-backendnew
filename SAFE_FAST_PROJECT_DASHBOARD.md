...
- QQQ CFB option context remains blocked by missing setup-time-safe same-contract open interest for `QQQ   260427C00615000` / `instrument_id=1023411456`; the current local source audit found no same-contract `stat_type=9` row in the full-day statistics file and no rows in the targeted setup-window statistics file. The target contract listing audit confirms the contract was listed before setup on Apr 13 but absent from the local Apr 10 parent definitions, so any listing-aware exception would require a new explicit human rule decision with regression fixtures. The accepted first gate still requires open interest and does not allow volume-only liquidity to pass.
...
Use the implemented selector only as a regression-protected contract-selection rule. The next bounded option-context step is either acquire a timestamp-safe same-contract open-interest source for `instrument_id=1023411456` / `QQQ   260427C00615000` or explicitly authorize a new human rule decision with regression fixtures before changing the open-interest gate. Any next step must still avoid backtest, evidence fill, P&L, proof, profitability, readiness, or trade-plan result counting unless separately authorized.
...
- Option-context selector evidence status: local Databento application attempted. The initial ten-minute quote file had no setup-time-safe quote for top-ranked contract `QQQ   260427C00615000` expiring `2026-04-27` at strike `615`. The wider top-contract TCBBO rerun finds setup-time-safe quotes for `instrument_id=1023411456`. The new top-contract trades/statistics rerun finds setup-time-safe trade volume `65`, curing the trade-volume gate, but the new statistics file contains no rows and cannot satisfy timestamp-safe same-contract open interest. `SAFE_FAST_DAY41_QQQ_CFB_OPEN_INTEREST_GATE_DECISION.md` keeps setup-time-safe same-contract open interest required, rejects volume-only liquidity for this first rule, and keeps `option_context_status=unknown`. `SAFE_FAST_DAY41_QQQ_CFB_OPEN_INTEREST_SOURCE_AUDIT.md` confirms the current local full-day statistics file has open-interest rows for other contracts but no same-contract `stat_type=9` rows for the target contract.
