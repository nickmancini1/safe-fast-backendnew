...
Batch-mode the remaining parked/replace candidates instead of continuing to grind QQQ CFB. Cheap starter Databento files are now locally validated and inspected for all six remaining candidates. Next grouped target: SPY Clean Fast Break rule/regression package for `SPY-REAL-HISTORICAL-CLEAN-FAST-BREAK-002` and `SPY-REAL-HISTORICAL-CLEAN-FAST-BREAK-003`, using starter data only for later read-only raw inspection after the SPY CFB rules exist.
...
The cheap starter Databento batch validation and starter option inspection are complete. The next single action is the grouped SPY CFB rule/regression task described in `SAFE_FAST_DAY41_STARTER_BATCH_NEXT_GROUPED_TASK.md`. Do not download full-window data, fill evidence, backtest, calculate P&L, claim proof/profitability, or mark readiness from the starter inspection alone.
