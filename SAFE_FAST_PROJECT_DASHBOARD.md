...
Execute `SAFE_FAST_DAY47_CFB_EXECUTION_REALISM_RULES_CODEX_TASK.md`. The promotion, final-outcome, sample-size, protected holdout, and candidate/contract freeze rules are now defined in `SAFE_FAST_PROJECT_PROOF_PIPELINE.md` and recorded in `SAFE_FAST_DAY47_PROMOTION_HOLDOUT_AND_CANDIDATE_FREEZE_RULES_RESULT.md`. Do not download more data, run new backtests, claim proof/profitability, mark readiness, or treat replay output as live trade evaluation.
