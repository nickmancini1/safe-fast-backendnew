...
Create data-only regression fixtures for the accepted QQQ CFB execution-context rule, including the target stale-quote case expected to return `fail`. Any next step must still avoid evidence fill, backtest, P&L, proof, profitability, readiness, or trade-plan result counting unless separately authorized.
...
- Execution-context rule status: first conservative rule accepted for regression work only. Quote age `<= 60` seconds is `clean`, quote age `> 60` seconds and `<= 5` minutes is `caution`, quote age `> 5` minutes or quote/spread/size/volume/no-fallback failure is `fail`, and missing source/rule proof is `unknown`. Later long-call testing uses ask price only as the fill basis. The target quote at `2026-04-13T16:06:30.640301037Z` is about `23` minutes `29` seconds old at setup, so expected target execution status after fixtures and implementation is `fail`.
