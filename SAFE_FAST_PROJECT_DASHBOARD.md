...
- QQQ CFB option context remains blocked by missing setup-time-safe same-contract open interest for `QQQ   260427C00615000` / `instrument_id=1023411456`; the accepted first gate still requires open interest and does not allow volume-only liquidity to pass.
...
Use the implemented selector only as a regression-protected contract-selection rule. The next bounded option-context step is either acquire a timestamp-safe same-contract open-interest source for `instrument_id=1023411456` or explicitly authorize a new human rule decision with regression fixtures before changing the open-interest gate. Any next step must still avoid backtest, evidence fill, P&L, proof, profitability, readiness, or trade-plan result counting unless separately authorized.
