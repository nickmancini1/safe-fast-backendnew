...
Run `SAFE_FAST_DAY47_GROUPED_CFB_SELECTED_CONTRACT_REPLAY_BACKTEST_CODEX_TASK.md` to replay the downloaded `SPY-REAL-HISTORICAL-CLEAN-FAST-BREAK-003` selected-contract evidence while preserving the grouped CFB anchors and no-trade controls. After the current grouped data/replay path is complete, run the mandatory queued audit task `SAFE_FAST_DAY47_TO_DAY90_CONSOLIDATED_AUDIT_AND_COMPLETION_PLAN_CODEX_TASK.md`. Do not download more data, claim proof/profitability, mark readiness, or treat the download as live trade evaluation.
