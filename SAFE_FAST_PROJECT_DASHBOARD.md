...
Batch-mode the remaining parked/replace candidates instead of continuing to grind QQQ CFB. Cheap starter Databento files are now locally validated and inspected for all six remaining candidates. The first SPY Clean Fast Break lifecycle rule/regression package, calculator coverage, and lifecycle-only work-package fills now exist for `SPY-REAL-HISTORICAL-CLEAN-FAST-BREAK-002` and `SPY-REAL-HISTORICAL-CLEAN-FAST-BREAK-003`. Starter data may be used only for later read-only raw option inspection after explicit authorization; contract/trade selection, context/caution evidence fill, backtest, P&L, proof, profitability, and readiness remain separate blocked tasks.
...
The cheap starter Databento batch validation, starter option inspection, SPY CFB grouped lifecycle rule/regression package, lifecycle calculator coverage, and lifecycle-only work-package fills are complete. The next single action, if explicitly authorized later, is read-only SPY CFB starter option inspection under SPY-specific contract/context rules, or a separate SPY CFB context/caution rule package. Do not download full-window data, backtest, calculate P&L, claim proof/profitability, or mark readiness from the lifecycle fill or starter inspection alone.
