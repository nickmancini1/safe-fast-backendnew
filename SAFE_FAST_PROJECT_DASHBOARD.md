...
Batch-mode the remaining parked/replace candidates instead of continuing to grind QQQ CFB. Cheap starter Databento files are now locally validated and inspected for all six remaining candidates. The first SPY Clean Fast Break lifecycle rule/regression package now exists for `SPY-REAL-HISTORICAL-CLEAN-FAST-BREAK-002` and `SPY-REAL-HISTORICAL-CLEAN-FAST-BREAK-003`. Starter data may be used only for later read-only raw option inspection after explicit authorization; evidence fill, contract/trade selection, backtest, P&L, proof, profitability, and readiness remain separate blocked tasks.
...
The cheap starter Databento batch validation, starter option inspection, and SPY CFB grouped lifecycle rule/regression package are complete. The next single action, if explicitly authorized later, is either a SPY CFB lifecycle calculator implementation against `historical_signal_replay/fixtures/spy_cfb_lifecycle_regression_fixtures.json` or read-only starter option inspection under SPY-specific contract/context rules. Do not download full-window data, fill evidence, backtest, calculate P&L, claim proof/profitability, or mark readiness from this package or the starter inspection alone.
