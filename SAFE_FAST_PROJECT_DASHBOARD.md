...
- QQQ CFB option context remains recorded as `unknown` because evidence files and selector code were not changed. A new narrow listing-aware exception is accepted for regression work, but regression fixtures and selector/rule implementation are still needed before the expected target result can move to `caution`.
