...
- QQQ CFB option context remains blocked by missing setup-time-safe same-contract open interest for `QQQ   260427C00615000` / `instrument_id=1023411456`; the current local source audit found no same-contract `stat_type=9` row in the full-day statistics file and no rows in the targeted setup-window statistics file. The accepted first gate still requires open interest and does not allow volume-only liquidity to pass.
