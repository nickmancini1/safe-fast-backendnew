...
| Stale/spent rules | partially accepted; needs calculator/evidence fill and non-QQQ decisions | QQQ Clean Fast Break now has a first accepted same-candle lifecycle decision in `SAFE_FAST_DAY41_QQQ_CFB_STALE_SPENT_EXPIRY_DECISION.md` and data-only lifecycle regression fixtures in `historical_signal_replay/fixtures/qqq_cfb_lifecycle_regression_fixtures.json`, but the lifecycle calculator and evidence fill are still missing. Other setup-specific stale/spent rules remain undecided. |
...
| QQQ CFB lifecycle calculator evidence use | pending validation | Data-only lifecycle regression fixtures now exist, but no lifecycle calculator has been created and no stale/spent/expiry evidence fields have been filled. |
