...
| Stale/spent rules | partially accepted; needs evidence fill and non-QQQ decisions | QQQ Clean Fast Break now has a first accepted same-candle lifecycle decision in `SAFE_FAST_DAY41_QQQ_CFB_STALE_SPENT_EXPIRY_DECISION.md`, data-only lifecycle regression fixtures in `historical_signal_replay/fixtures/qqq_cfb_lifecycle_regression_fixtures.json`, and a focused lifecycle calculator in `historical_signal_replay/cfb_lifecycle_calculator.py`, but lifecycle evidence fill is still missing. Other setup-specific stale/spent rules remain undecided. |
...
| QQQ CFB lifecycle calculator evidence use | pending validation | The lifecycle calculator now passes all 18 data-only lifecycle regression fixtures, but no stale/spent/expiry evidence fields have been filled. |
