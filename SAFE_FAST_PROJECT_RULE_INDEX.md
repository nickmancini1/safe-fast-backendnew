...
| Stale/spent rules | partially accepted; needs fixtures and non-QQQ decisions | QQQ Clean Fast Break now has a first accepted same-candle lifecycle decision in `SAFE_FAST_DAY41_QQQ_CFB_STALE_SPENT_EXPIRY_DECISION.md`, but data-only lifecycle regression fixtures are still missing before evidence fill. Other setup-specific stale/spent rules remain undecided. |
