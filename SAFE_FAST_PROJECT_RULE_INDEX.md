...
| Stale/spent rules | missing/needs decision | Setup-specific rules and regression fixtures for stale/spent lifecycle states. For QQQ Clean Fast Break, `SAFE_FAST_DAY41_QQQ_CFB_STALE_SPENT_EXPIRY_DECISION_NEEDED.md` requires an accepted choice for freshness window, stale timing, spent behavior, expiry clock, higher-base refresh, and state precedence before evidence fill. |
