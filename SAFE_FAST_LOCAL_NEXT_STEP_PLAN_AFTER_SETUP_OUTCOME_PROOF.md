# SAFE-FAST Local Next-Step Plan After Setup Outcome Proof

## 1. Plain-English Purpose

SAFE-FAST can now say what happened after a setup appeared. The next small build step should explain why the result happened, why proof was limited, and what the smallest responsible fix path is before any optimization.

This plan is for a local-only diagnostics step that turns setup outcome proof results into clear failure reasons and next fix paths. It is not live trade chat and does not start live trading, controlled shadow data, alerts, watcher loops, reports, or optimization.
...
- **Latest local setup outcome proof milestone:** `b878cae Add setup outcome proof evaluator`.
- **Work mode:** SAFE-FAST build work only, not live trade decisions.
- **Planning scope:** docs-only local next-step planning after the setup outcome proof evaluator.
...
- The setup outcome proof evaluator accepts caller-provided in-memory setup outcome proof records, requires frozen setup identity before after-setup evidence, preserves no-hindsight/no-trade/watch-only boundaries, classifies each accepted record into one setup outcome status, marks proof-limited records, and emits initial diagnostic/fix-path placeholders.
...
- It is not yet proven whether successful setup outcomes are repeatable enough to support a viable plan.
- It is not yet proven whether setup outcome proof gaps require lower-tier handoff before higher-tier watcher changes.
...
Add one local-only setup outcome diagnostics evaluator.

The future evaluator should accept the in-memory result from `evaluate_setup_outcome_proof(...)` and return one in-memory diagnostics summary that explains each accepted or rejected proof result in terms of:

- evidence used
- unavailable or missing evidence
- outcome status
- affected setup type
- affected symbol when available
- affected stage
- trigger/invalidation/freshness/session relationship
...
The evaluator must preserve diagnostics-before-optimization. It may identify candidate fix paths, but it must not change rules, tune thresholds, optimize rankings, start watcher behavior, produce generated reports/logs, or make viability claims.

## 6. Allowed Files for That Future Step

- `watcher_foundation/setup_outcome_diagnostics.py`
- `watcher_foundation/__init__.py`
- `tests/test_setup_outcome_diagnostics.py`
...
Add focused local unit tests in `tests/test_setup_outcome_diagnostics.py` covering:

- Accepts only the in-memory setup outcome proof summary shape.
- Requires `watch_only=True`, `setup_outcome_proof_only=True`, `final_viability_proven=False`, and `optimization_started=False`.
- Rejects summaries that started live data, controlled shadow data, alerts, file writes, broker/trade behavior, or optimization.
- Preserves accepted proof records and rejected proof reasons as evidence only.
- Emits diagnostics for `triggered_worked` without claiming viability.
- Emits diagnostics for `triggered_failed` with outcome-scoring or trigger/invalidation fix path.
- Emits diagnostics for `triggered_inconclusive`.
- Emits diagnostics for `stayed_valid_pending`.
- Emits diagnostics for `stale_without_trigger`.
- Emits diagnostics for `invalidated_before_trigger`.
- Emits diagnostics for `insufficient_evidence`.
- Carries proof-limited fields into missing-evidence diagnostics.
- Produces diagnostic gap counts and next fix paths.
- Marks lower-tier handoff required when proof is insufficient, rejected, missing source evidence, or blocked by trigger/invalidation/freshness ambiguity.
...
- Implementation status for the local-only setup outcome diagnostics evaluator.
...
- Next local-only objective after setup outcome diagnostics.
...
Detection alone is not enough. Outcome labels alone are not enough. The next step must explain why each outcome happened and what local contract, fixture, test, or rule area should be reviewed next.
...
No threshold, ranking, trigger, invalidation, freshness, blocker/caution, or workflow optimization may be proposed until diagnostic output identifies an evidence-backed rule, contract, or test gap.
...
If setup outcome diagnostics find insufficient evidence, stale setup handling gaps, trigger/invalidation ambiguity, blocker/caution ambiguity, session-boundary ambiguity, diagnostics gaps, or workflow ambiguity, the output must identify whether the issue requires lower-tier handoff before any higher-tier optimization or live-readiness step.
