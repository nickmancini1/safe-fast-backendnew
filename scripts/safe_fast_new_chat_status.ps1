...
Write-Host "PRIORITY_DRIFT_GATE: Before every response, answer Priority check YES/NO; name current priority; label the step Substance or busy work; explain why allowed; if NO, stop and correct before a command."
Write-Host "PRIORITY_VERTICAL_EXECUTION_GATE: Substance commands must complete result, tests, diff check, commit, final decision, next action, latest commit, and clean status; read-only gates are allowed only for dirty status, failed tests, paid/vendor approval, missing auth, unknown mutating script arguments, or missing/unparseable machine result."
Write-Host "PRIORITY_ALLOWED_STEPS: cost approval; vendor-ready request; contract selection; option evidence; entry/exit/P&L; exact rejection; exact blocker closure; regression protection."
...
Write-Host "CODEX_GUARDRAIL: Run Codex through codex.cmd with windows.sandbox=""unelevated""; Codex tasks must be sealed tickets."
Write-Host "RAW_REPLAY_SOURCE: Databento supplies raw historical replay evidence."
Write-Host "TRADING_WORKFLOW_SOURCE: Tastytrade/dxLink is candidate/trading workflow source only where the repo/task says so."
Write-Host "MACRO_EVENT_SOURCE: Official agencies / ALFRED supply macro and event facts."
Write-Host "SAFE_FAST_DECISION_OWNERSHIP: SAFE-FAST owns labels, entry, exit, P&L, profitability, paper eligibility, and live eligibility."
Write-Host "WEAK_OR_INCONCLUSIVE_RULE: If SAFE-FAST is unprofitable, weak, missing evidence, or inconclusive, diagnose the exact cause and smallest evidence-backed fix."
Write-Host "CANDIDATE_FACTORY: Candidate work is a factory, not a random hunt; replay-ready candidates go directly to economics/P&L."
