...
FIRST_REAL_QQQ_CONTINUATION_FIXTURE_PATH = (
    BASE_DIR / "fixtures" / "first_real_qqq_continuation_replay_v1_fixture.json"
)
FIRST_REAL_QQQ_CONTINUATION_SIGNAL_LOG_PATH = (
    REPORTS_DIR / "first_real_qqq_continuation_replay_v1_signal_log.jsonl"
)
FIRST_REAL_QQQ_CONTINUATION_SUMMARY_PATH = (
    REPORTS_DIR / "first_real_qqq_continuation_replay_v1_summary.json"
)
FIRST_REAL_QQQ_CONTINUATION_REGRESSION_CANDIDATES_PATH = (
    REPORTS_DIR / "first_real_qqq_continuation_replay_v1_regression_candidates.json"
)
...
def _build_first_real_qqq_continuation_regression_candidates(rows):
    return {
        "purpose": (
            "First-real QQQ Continuation signal/stage/lifecycle replay "
            "regression candidates only; no profitability, P&L, account sizing, "
            "trade outcomes, live trade decisions, or auto-trading."
        ),
        "candidates": [
...
def run_first_real_qqq_continuation_replay():
    fixture = _load_json(FIRST_REAL_QQQ_CONTINUATION_FIXTURE_PATH)
    validate_lifecycle_fixture(fixture)
    ...
    summary = build_lifecycle_summary(signal_rows)
    regression_candidates = (
        _build_first_real_qqq_continuation_regression_candidates(signal_rows)
    )

    REPORTS_DIR.mkdir(parents=True, exist_ok=True)
    _write_signal_log(FIRST_REAL_QQQ_CONTINUATION_SIGNAL_LOG_PATH, signal_rows)
    FIRST_REAL_QQQ_CONTINUATION_SUMMARY_PATH.write_text(
        json.dumps(summary, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    FIRST_REAL_QQQ_CONTINUATION_REGRESSION_CANDIDATES_PATH.write_text(
        json.dumps(regression_candidates, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    ...
    return {
        ...
        "summary": summary,
        "regression_candidates": regression_candidates,
        "signal_log_path": str(FIRST_REAL_QQQ_CONTINUATION_SIGNAL_LOG_PATH),
        "summary_path": str(FIRST_REAL_QQQ_CONTINUATION_SUMMARY_PATH),
        "regression_candidates_path": str(
            FIRST_REAL_QQQ_CONTINUATION_REGRESSION_CANDIDATES_PATH
        ),
    }
...
def main():
    ...
    first_real_qqq_continuation_result = run_first_real_qqq_continuation_replay()
    ...
    print("First-real QQQ Continuation replay complete")
    ...
    print(
        "First-real QQQ Continuation signal log: "
        f"{first_real_qqq_continuation_result['signal_log_path']}"
    )
    print(
        "First-real QQQ Continuation summary: "
        f"{first_real_qqq_continuation_result['summary_path']}"
    )
    print(
        "First-real QQQ Continuation regression candidates: "
        f"{first_real_qqq_continuation_result['regression_candidates_path']}"
    )
