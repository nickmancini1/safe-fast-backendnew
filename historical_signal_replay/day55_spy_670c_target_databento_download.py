...
MANIFEST_PATH = OUTPUT_ROOT / "day55_spy_670c_target_download_manifest.json"

ENV_VAR_NAME = "SAFE_FAST_DB_AUTH"
DATASET = "OPRA.PILLAR"
EXACT_SYMBOL = "SPY   260330C00670000"
APPROVED_TARGET_COST = Decimal("0.006495481730")
EXPECTED_REQUEST_COUNT = 4
REQUIRED_SCHEMAS = ("cmbp-1", "tcbbo", "trades", "statistics")
FORBIDDEN_SCHEMAS = {"definition"}
EXPECTED_DECISION = "TARGET_COST_ONLY_REQUEST_READY_FOR_OPERATOR_APPROVAL"
RESULT_VERSION = "safe_fast_day55_spy_670c_target_download_manifest_v1"
...
def load_source_request(*, source_request_path=SOURCE_REQUEST_PATH):
    return load_json(source_request_path)
...
def validate_preflight(source_request, *, output_root=OUTPUT_ROOT):
    problems = []
    if source_request.get("decision") != EXPECTED_DECISION:
        problems.append("source_request_decision_unexpected")
    if source_request.get("exact_symbol") != EXACT_SYMBOL:
        problems.append("exact_symbol_mismatch")
    if source_request.get("dataset") != DATASET:
        problems.append("dataset_mismatch")
    if source_request.get("request_count") != EXPECTED_REQUEST_COUNT:
        problems.append("request_count_not_4")
    ...
    try:
        if Decimal(str(source_request.get("exact_estimated_cost"))) != APPROVED_TARGET_COST:
            problems.append("exact_estimated_cost_not_operator_approved_amount")
    except (InvalidOperation, TypeError):
        problems.append("exact_estimated_cost_not_decimal")

    destination = source_request.get("destination_for_approved_download")
    if destination != _relative(OUTPUT_ROOT):
        problems.append("destination_for_approved_download_mismatch")
    if Path(output_root).resolve() != OUTPUT_ROOT.resolve():
        problems.append("output_root_must_be_target_only_destination")

    if tuple(source_request.get("required_schemas") or []) != REQUIRED_SCHEMAS:
        problems.append("required_schemas_mismatch")
    if set(source_request.get("forbidden_schemas") or []) != FORBIDDEN_SCHEMAS:
        problems.append("forbidden_schemas_mismatch")

    requests = [vendor_request(request) for request in deepcopy(source_request.get("requests") or [])]
    if len(requests) != EXPECTED_REQUEST_COUNT:
        problems.append("request_list_length_not_4")
    if [request.get("schema") for request in requests] != list(REQUIRED_SCHEMAS):
        problems.append("request_schema_order_mismatch")
    if len({request_key(request) for request in requests}) != EXPECTED_REQUEST_COUNT:
        problems.append("request_keys_not_unique")

    for index, request in enumerate(requests):
        if sorted(request.keys()) != ["dataset", "end", "schema", "start", "stype_in", "symbols"]:
            problems.append("request_contains_non_vendor_fields")
        if request.get("dataset") != DATASET:
            problems.append("request_dataset_mismatch")
        if request.get("schema") in FORBIDDEN_SCHEMAS:
        ...
        if request.get("symbols") != EXACT_SYMBOL:
        ...
        if not request.get("start") or not request.get("end"):
            problems.append(f"request_{index}_missing_window")

    schema_costs = source_request.get("schema_costs") or []
    if [vendor_request(row) for row in schema_costs] != requests:
...
def request_id(index, request):
    ...
    schema = request["schema"].replace("-", "")
    return f"{index + 1:02d}_{symbol}_{schema}_{start}_{end}"
...
def inspect_existing_outputs(requests, *, output_root=OUTPUT_ROOT, dbn_parser=None):
    by_id = {}
    paths_by_id = planned_output_files(requests, output_root=output_root)
    for index, request in enumerate(requests):
        rid = request_id(index, request)
...
def validate_evidence_pair(request, paths, *, request_id, request_index, dbn_parser=None):
    ...
    if csv_summary["record_count"] > 0 and csv_summary.get("symbol") != request["symbols"]:
...
def download_missing_request(api_key, request, *, request_id, request_index, client, output_root=OUTPUT_ROOT, dbn_parser=None):
    if not api_key:
...
def build_manifest(
    *,
    ...
    request_list = deepcopy(requests or source_request.get("requests", []))
    output_files = [
    ...
    return {
        ...
        "operator_approved_target_cost_usd": str(APPROVED_TARGET_COST),
        "checked_target_cost_usd": source_request.get("exact_estimated_cost"),
        ...
        "download_performed": any(row.get("status") == "COMPLETED_DOWNLOADED" for row in result_map.values()),
...
def run_download(
    *,
    ...
    source_request = load_source_request(source_request_path=source_request_path)
    requests = source_request.get("requests", [])
    request_results = {}
    current_request_id = None
    try:
        requests = validate_preflight(source_request, output_root=output_root)
        Path(output_root).mkdir(parents=True, exist_ok=True)
        request_results = inspect_existing_outputs(requests, output_root=output_root, dbn_parser=dbn_parser)
        _write_checkpoint("IN_PROGRESS", source_request, requests, request_results, manifest_path, output_root, current_request_id=None)
        for index, request in enumerate(requests):
            rid = request_id(index, request)
            current_request_id = rid
            if request_results[rid]["status"] == "COMPLETED_REUSED":
                _emit(stream, f"REQUEST_REUSED={rid}")
                _write_checkpoint("IN_PROGRESS", source_request, requests, request_results, manifest_path, output_root, current_request_id=rid)
                continue
            ...
            _write_checkpoint("IN_PROGRESS", source_request, requests, request_results, manifest_path, output_root, current_request_id=rid)
        status = "SUCCESS" if _all_final(request_results, requests) else "FAILURE"
        failure = None if status == "SUCCESS" else {
            "classification": "INCOMPLETE_REQUEST_SET",
            "detail": "not all 4 target requests are completed or reused",
        }
...
def _write_checkpoint(status, source_request, requests, request_results, manifest_path, output_root, *, current_request_id):
    manifest = build_manifest(
        status=status,
        source_request=source_request,
        requests=requests,
