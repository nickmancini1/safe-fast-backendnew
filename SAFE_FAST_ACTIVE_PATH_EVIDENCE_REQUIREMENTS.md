...
The acquisition request converts those 9 gap rows into exact requests for source/export type, timestamp/session/window, required fields, reason, rule resolved, current-data availability, and post-acquisition action. It does not reactivate any row.
