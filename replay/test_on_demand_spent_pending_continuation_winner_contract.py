...
spent_pending_continuation = candidate(
    symbol="QQQ",
...
ranked = sorted(
    [spent_pending_continuation, fresh_pending_ideal],
...
print(
    "spent_pending_continuation_vs_fresh_pending_ideal: "
    f"selected={actual}, expected=SPY, ranked={[item.get('symbol') for item in ranked]}"
)

if actual != "SPY":
    print("SPENT PENDING CONTINUATION WINNER CONTRACT FAILURES: expected_SPY_got_" + str(actual))
    raise SystemExit(2)

print("SPENT PENDING CONTINUATION WINNER CONTRACT PASS")
