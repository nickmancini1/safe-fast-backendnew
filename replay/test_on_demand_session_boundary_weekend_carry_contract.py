...
friday_break_monday_back_below = [
    candle("2026-05-01T09:30:00-04:00", 99.8, 100.2, 99.7, 100.0),
    candle("2026-05-01T10:30:00-04:00", 100.0, 100.3, 99.9, 100.1),
    candle("2026-05-01T11:30:00-04:00", 100.35, 100.65, 100.25, 100.55),
    candle("2026-05-01T12:30:00-04:00", 100.50, 100.70, 100.35, 100.60),
    candle("2026-05-01T13:30:00-04:00", 100.55, 100.75, 100.45, 100.65),
    candle("2026-05-01T14:30:00-04:00", 100.65, 101.00, 100.60, 100.85),
    candle("2026-05-04T09:30:00-04:00", 100.40, 100.50, 100.10, 100.25),
    candle("2026-05-04T10:30:00-04:00", 100.25, 100.35, 100.00, 100.20),
]

continuation_context = continuation_context_for(friday_break_monday_back_below)
trigger_state = trigger_state_for(friday_break_monday_back_below, continuation_context)
...
print("weekend_prior_session continuation:", compact_continuation)
print("weekend_prior_session trigger_state:", compact_trigger)

failures = []

if continuation_context.get("prior_completed_shelf_break_seen") is not True:
    failures.append("weekend_prior_session_break_not_detected")

if continuation_context.get("exact_reason") not in {"spent", "late"}:
    failures.append("weekend_prior_session_break_should_be_spent_or_late")

if continuation_context.get("tradeable_now") is True:
    failures.append("weekend_prior_session_break_should_not_be_tradeable_now")

if continuation_context.get("inside_tradeable_window") is True:
    failures.append("weekend_prior_session_break_should_not_be_inside_tradeable_window")

message = str(continuation_context.get("status_message") or "").lower()
if "waiting for the first completed" in message:
    failures.append("weekend_prior_session_break_should_not_say_waiting_for_first_break")

if trigger_state.get("why") != "prior_completed_shelf_break_spent":
    failures.append("weekend_prior_session_break_should_surface_spent_trigger_why")

if trigger_state.get("trigger_style") != "prior_session_completed_shelf_break_spent":
    failures.append("weekend_prior_session_break_should_surface_spent_trigger_style")

if trigger_state.get("completed_candle_trigger_present") is True:
    failures.append("weekend_prior_session_break_should_not_be_completed_candle_trigger_present")

if trigger_state.get("structure_ready") is True:
    failures.append("weekend_prior_session_break_should_not_make_structure_ready")

if failures:
    print("SESSION BOUNDARY WEEKEND CARRY CONTRACT FAILURES:", ", ".join(failures))
    raise SystemExit(2)

print("SESSION BOUNDARY WEEKEND CARRY CONTRACT PASS")
