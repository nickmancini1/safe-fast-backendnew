...
def assert_prior_session_break_not_fresh(name, candles):
    continuation_context = continuation_context_for(candles)
    trigger_state = trigger_state_for(candles, continuation_context)
    ...
    print(f"{name} continuation:", compact_continuation)
    print(f"{name} trigger_state:", compact_trigger)

    failures = []

    if continuation_context.get("prior_completed_shelf_break_seen") is not True:
        failures.append(f"{name}:prior_completed_shelf_break_not_detected")

    if continuation_context.get("exact_reason") not in {"spent", "late"}:
        failures.append(f"{name}:prior_break_should_be_spent_or_late")

    if continuation_context.get("tradeable_now") is True:
        failures.append(f"{name}:prior_break_should_not_be_tradeable_now")

    if continuation_context.get("inside_tradeable_window") is True:
        failures.append(f"{name}:prior_break_should_not_be_inside_tradeable_window")
    ...
    if trigger_state.get("structure_ready") is True:
        failures.append(f"{name}:prior_break_should_not_make_structure_ready")

    return failures


prior_session_break_back_below = [
    candle("2026-05-06T09:30:00-04:00", 99.8, 100.2, 99.7, 100.0),
    candle("2026-05-06T10:30:00-04:00", 100.0, 100.3, 99.9, 100.1),
    ...
    candle("2026-05-07T09:30:00-04:00", 100.40, 100.50, 100.10, 100.25),
    candle("2026-05-07T10:30:00-04:00", 100.25, 100.35, 100.00, 100.20),
]

prior_session_break_still_above = [
    candle("2026-05-06T09:30:00-04:00", 99.8, 100.2, 99.7, 100.0),
    candle("2026-05-06T10:30:00-04:00", 100.0, 100.3, 99.9, 100.1),
    candle("2026-05-06T11:30:00-04:00", 100.35, 100.65, 100.25, 100.55),
    candle("2026-05-06T12:30:00-04:00", 100.50, 100.70, 100.35, 100.60),
    candle("2026-05-06T13:30:00-04:00", 100.55, 100.75, 100.45, 100.65),
    candle("2026-05-06T14:30:00-04:00", 100.65, 101.00, 100.60, 100.85),
    candle("2026-05-07T09:30:00-04:00", 100.75, 100.95, 100.65, 100.82),
    candle("2026-05-07T10:30:00-04:00", 100.82, 100.98, 100.70, 100.88),
]

failures = []
failures.extend(
    assert_prior_session_break_not_fresh(
        "prior_session_completed_break_back_below_trigger",
        prior_session_break_back_below,
    )
)
failures.extend(
    assert_prior_session_break_not_fresh(
        "prior_session_completed_break_still_above_trigger",
        prior_session_break_still_above,
    )
)

if failures:
    print("SESSION BOUNDARY CONTRACT FAILURES:", ", ".join(failures))
    raise SystemExit(2)

print("SESSION BOUNDARY CONTRACT PASS")