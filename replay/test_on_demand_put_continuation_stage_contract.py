...
def chart_check(completed_close, current_close):
    candles = [
        candle("2026-05-07T10:30:00-04:00", 99.90),
    ...
    return {
        ...
        "ema50_1h": 100.0,
        "price_vs_ema50_1h": "below" if current_close < 100.0 else "above",
    }


def structure_context(continuation_overrides=None):
    continuation = {
        "shelf_exists": True,
        "shelf_proven": True,
...
put_intrabar_waiting = _build_trigger_state(
    "P",
    {"is_open": True},
    {"fresh_entry_allowed": True},
    structure_context(),
    chart_check(completed_close=99.60, current_close=99.20),
)
failures.extend(
    f"put_intrabar_waits_completed_close:{item}"
    for item in assert_case(
        "put_intrabar_waits_completed_close",
        {
            "trigger_present": put_intrabar_waiting.get("trigger_present"),
            "structural_trigger_present": put_intrabar_waiting.get("structural_trigger_present"),
            "completed_candle_trigger_present": put_intrabar_waiting.get("completed_candle_trigger_present"),
            "pending_completed_candle_approval": put_intrabar_waiting.get("pending_completed_candle_approval"),
            "trigger_style": put_intrabar_waiting.get("trigger_style"),
            "why": put_intrabar_waiting.get("why"),
        },
        {
...
put_completed_market_closed = _build_trigger_state(
    "P",
    {"is_open": False},
    ...
    chart_check(completed_close=99.20, current_close=99.10),
)
failures.extend(
    f"put_completed_trigger_market_closed_not_live:{item}"
    for item in assert_case(
        "put_completed_trigger_market_closed_not_live",
        {
            "trigger_present": put_completed_market_closed.get("trigger_present"),
            "structural_trigger_present": put_completed_market_closed.get("structural_trigger_present"),
            "completed_candle_trigger_present": put_completed_market_closed.get("completed_candle_trigger_present"),
            "structure_ready": put_completed_market_closed.get("structure_ready"),
            "live_entry_requires_market_open": put_completed_market_closed.get("live_entry_requires_market_open"),
            "live_entry_waiting_on": put_completed_market_closed.get("live_entry_waiting_on"),
            "why": put_completed_market_closed.get("why"),
        },
        {
            "trigger_present": False,
...
if failures:
    print("PUT CONTINUATION STAGE CONTRACT FAILURES:", ", ".join(failures))
    raise SystemExit(2)

print("PUT CONTINUATION STAGE CONTRACT PASS")
