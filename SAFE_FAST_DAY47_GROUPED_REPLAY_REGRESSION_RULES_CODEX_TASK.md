# SAFE-FAST Day 47 Grouped Replay Regression Rules - Codex Task
...
2. `SAFE_FAST_DAY47_DATA_COST_LEDGER_RULES_RESULT.md`
3. `SAFE_FAST_DAY47_PORTFOLIO_INTERACTION_RULES_RESULT.md`
4. `SAFE_FAST_DAY47_RISK_CAPITAL_RULES_RESULT.md`
5. `SAFE_FAST_DAY47_TO_DAY90_CONSOLIDATED_AUDIT_AND_COMPLETION_PLAN_RESULT.md`
6. `SAFE_FAST_PROJECT_DASHBOARD.md`
7. `SAFE_FAST_PROJECT_RULE_INDEX.md`
8. `SAFE_FAST_PROJECT_PROOF_PIPELINE.md`

Expected starting state:

- branch: `main`
- commit: current local HEAD after the Day 47 data-cost ledger package is committed, if the user commits it
...
Implement the next grouped Day 47-to-Day 90 rule package identified by the consolidated audit: grouped replay/regression expansion governance after promotion/outcome/sample/holdout/freeze, CFB execution-realism, risk/capital, portfolio-interaction, and data-cost ledger rules are defined.
...
Define conservative grouped replay/regression rules for:
...
- accepted/rejected/ambiguous/no-trade case handling;
...
For every changed or accepted grouped replay/regression rule, state:
...
- Do not silently alter an existing accepted CFB trading rule, execution-realism rule, risk/capital rule, portfolio-interaction rule, or data-cost ledger rule.
...
5. focused tests for the new grouped replay/regression rule package and control-file agreement;
6. `git diff --check`.
