# SAFE-FAST Day 47 Risk Capital Rules - Codex Task
...
2. `SAFE_FAST_DAY47_CFB_EXECUTION_REALISM_RULES_RESULT.md`
3. `SAFE_FAST_DAY47_PROMOTION_HOLDOUT_AND_CANDIDATE_FREEZE_RULES_RESULT.md`
...
- branch: `main`
- commit: current local HEAD after the Day 47 CFB execution-realism package is committed, if the user commits it
...
Implement the next grouped Day 47-to-Day 90 rule package identified by the consolidated audit and by `SAFE_FAST_DAY47_CFB_EXECUTION_REALISM_RULES_RESULT.md`: risk and capital rules before portfolio interaction or more countable results.
...
Define conservative risk and capital rules for:
...
- position sizing placeholders for replay, protected holdout, paper validation, and live-review eligibility;
...
For every changed or accepted risk/capital rule, state:
...
- Do not silently alter an existing accepted CFB trading rule or execution-realism rule.
...
5. focused tests for the new risk/capital rule package and control-file agreement;
6. `git diff --check`.
