# SAFE-FAST Day 50 Positive-Entry Active-Path Rule Evidence Repair
...
1. `SAFE_FAST_DAY50_POSITIVE_ENTRY_TRADE_CANDIDATE_RULE_GAP_CLOSEOUT_RESULT.md`
2. `historical_signal_replay/results/day50_positive_entry_trade_candidate_rule_gap_closeout.json`
3. `SAFE_FAST_DAY50_POSITIVE_ENTRY_CONTRACT_SELECTED_MISSING_EVIDENCE_RESULT.md`
4. `historical_signal_replay/results/day50_positive_entry_contract_selected_missing_evidence.json`
5. `SAFE_FAST_RULE_FAMILY_DECISION_TABLE.md`
6. `SAFE_FAST_RULE_DECISION_SURVIVAL_MAP.md`
...
If local Git or canonical control files disagree with the trade-candidate rule-gap closeout result, stop and report the exact conflict.

## Objective

Create the next bounded active-path rule/evidence repair package from existing local fixture/source evidence only.

Use the trade-candidate rule-gap closeout as the source of truth. The affected `TRADE_CANDIDATE` rows remain blocked before selected-contract identity, and QQQ Ideal is resolved as `replace` / outside the narrowed Ideal path from accepted frozen evidence.
...
`SAFE_FAST_DB_AUTH` may be used only for exact cost checks if a valid paid-data request is created. This task should not create one. Never print, save, document, or echo it.

## Required outputs

Create a result file and machine-readable JSON for this active-path repair package.
