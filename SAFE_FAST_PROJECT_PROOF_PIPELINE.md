...
| Concurrent-position limits | Accepted for replay and holdout governance until portfolio rules exist. SAFE-FAST may count at most `1` open option position at a time across all setup families and symbols. Simultaneous accepted signals require one selected candidate and all others become blocked by capital competition until portfolio interaction rules exist. | Open/close timestamps, signal timestamps, setup family, symbol, selected contract, candidate precedence record, and exclusion reason for each simultaneous candidate. | No overlap; exact same timestamp; overlapping positions; missing exit timestamp; duplicate-family overlap; cross-symbol overlap. | More than one open position; missing exit timestamp; no deterministic precedence record for simultaneous candidates; counting multiple overlapping winners. | Existing rows are not promoted because portfolio interaction and precedence rules are not yet defined. | One-position concurrency is a capital placeholder, not a portfolio optimization result. |
...
| Can simultaneous candidates both count? | Only after a pre-outcome portfolio precedence rule exists and selects the capital slot winner. | Block or keep all competing cases review-only until portfolio interaction rules exist. |
...
Current known state: repo docs after the Day 47 grouped selected-contract replay, consolidated audit, promotion/holdout/candidate-freeze package, CFB execution-realism package, and risk/capital package show work-package content validation at `9` passed requests and `0` failed requests. Intake-ready remains controlled by the separate readiness gate. SAFE-FAST still needs portfolio interaction rules, data-cost ledger, grouped replay/regression, repair/retirement/invalidation thresholds, and a Day 90 decision package before any paper-validation or live-readiness claim.
