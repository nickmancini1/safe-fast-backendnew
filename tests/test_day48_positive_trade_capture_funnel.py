...
class Day48PositiveTradeCaptureFunnelTests(unittest.TestCase):
    def test_funnel_builds_required_scorecards_and_deterministic_hashes(self):
        ...
        downloaded_evidence = day49.build_setup_time_evidence()
        self.assertEqual(combined["valid_trades_captured"], 1)
        self.assertEqual(combined["true_no_trades"], 4 if downloaded_evidence else 2)
        self.assertEqual(combined["missing_data_cases"], 6 if downloaded_evidence else 8)
    ...
    def test_true_no_trade_missing_data_and_valid_capture_stay_separate(self):
        ...
        spy_ideal = by_id["second_real_spy_ideal_replay_v1_fixture"]
        ...
        if day49.build_setup_time_evidence():
            self.assertEqual(spy_ideal["final_classification"], "TRUE_NO_TRADE")
            self.assertEqual(spy_ideal["exact_blocker_code"], "quote_age_above_5_minutes")
            self.assertEqual(spy_ideal["blocker_category"], "real frozen-rule failure")
            self.assertEqual(spy_continuation["final_classification"], "TRUE_NO_TRADE")
            self.assertEqual(
                spy_continuation["exact_blocker_code"],
                "quote_age_above_5_minutes",
            )
            self.assertEqual(spy_continuation["blocker_category"], "real frozen-rule failure")
        else:
    ...
    def test_family_and_combined_scorecards_keep_positive_capture_visible(self):
        ...
        self.assertEqual(
            ...
            4 if downloaded_evidence else 6,
        )
        self.assertEqual(
            document["combined_scorecard"]["first_blocker_totals_by_funnel_stage"][
                "ENTRY_ELIGIBLE"
            ],
            4 if downloaded_evidence else 2,
        )
