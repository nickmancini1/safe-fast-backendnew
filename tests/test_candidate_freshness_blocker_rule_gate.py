...
class CandidateFreshnessBlockerRuleGateTests(unittest.TestCase):
    ...
    def test_spy_cfb_003_higher_base_fresh_break_expiry_blocks_intake_ready(self):
        ...
        self.assertIn(
            ...
            " ".join(gate.spy_cfb_003_survival_action()["exact_missing_evidence"]),
        )
        self.assertFalse(
    ...
    def test_spy_cfb_003_context_caution_missing_evidence_blocks_intake_ready(self):
        candidate_id = "SPY-REAL-HISTORICAL-CLEAN-FAST-BREAK-003"
        decisions = {
            row["rule_family"]: row["hard_decision"]
            for row in gate.build_rule_gate_result()["gate_by_candidate"][candidate_id]
    ...
    def test_spy_cfb_002_initial_break_expiry_blocks_intake_ready(self):
        ...
        self.assertIn(
            ...
            " ".join(gate.spy_cfb_002_survival_action()["exact_missing_evidence"]),
        )
        self.assertFalse(
            gate.candidate_can_promote(
                candidate_id,
    ...
    def test_spy_cfb_002_context_caution_missing_evidence_blocks_intake_ready(self):
        candidate_id = "SPY-REAL-HISTORICAL-CLEAN-FAST-BREAK-002"
        decisions = {
            row["rule_family"]: row["hard_decision"]
            for row in gate.build_rule_gate_result()["gate_by_candidate"][candidate_id]
