# SAFE-FAST GLD Ideal 001 Replay Fixture Specification Review
...
- Current required HEAD: `6550d8a Fix latest completed commit after GLD Ideal replay review`
- Current HEAD verified before edits: `6550d8a Fix latest completed commit after GLD Ideal replay review`
- Build state reviewed: yes
- GLD Ideal 001 real historical replay review reviewed: yes
- GLD Ideal 001 replay readiness review reviewed: yes
...
- Closest existing fixture specification pattern inspected: `SAFE_FAST_IWM_IDEAL_001_REPLAY_FIXTURE_SPECIFICATION_REVIEW.md`
...
- Latest required commit present: yes, `6550d8a Fix latest completed commit after GLD Ideal replay review`
...
- Source-only shape: 2026-05-04 decline into the `413.2801` window low, 2026-05-05 base with `417.9050` low and `421.1300` high, 2026-05-06 recovery opening at `430.1` and reaching `433.19`, 2026-05-07 follow-through to the `437.42` window high, and 2026-05-08 pullback/hold rows ending at `433.795`.

## Source CSV Facts

- Validated source CSV row count: 290.
- Validated source CSV span: `2026-03-23T09:30:00-04:00` to `2026-05-20T11:30:00-04:00`.
- Selected GLD Ideal row count: 35.
- Selected GLD Ideal timestamp span: `2026-05-04T09:30:00-04:00` to `2026-05-08T15:30:00-04:00`.
...
The future fixture JSON should prove GLD Ideal 001 signal/stage/lifecycle/trigger-card behavior against a real, bounded GLD 1H RTH source window. It must use exact source CSV candles, preserve no-hindsight cumulative row inputs, and keep unavailable context fields unconfirmed.
...
- Expected fixture file name/path if fixture creation is approved in the next task: `historical_signal_replay/fixtures/first_real_gld_ideal_replay_v1_fixture.json`
- Fixture type: first real historical GLD Ideal replay v1 fixture.
...
- Proposed fixture row count: 6, following the existing IWM Ideal fixture specification style.

Proposed lifecycle row roles for future fixture creation:

| Row | Timestamp | Proposed row role | Source-only basis |
| --- | --- | --- | --- |
| 1 | `2026-05-04T15:30:00-04:00` | `watching_ideal_retest_low_context` | Completed rows through 2026-05-04 include the `413.2801` window low and a `414.73` close; no 2026-05-05 base, 2026-05-06 recovery, or later follow-through is available. |
| 2 | `2026-05-05T15:30:00-04:00` | `watching_ideal_base_after_retest_developing` | Completed rows through 2026-05-05 show a base day with `417.905` low, `421.13` high, and `418.17` close; no 2026-05-06 recovery is available. |
| 3 | `2026-05-06T09:30:00-04:00` | `ideal_recovery_confirmation_candidate` | Completed rows through 2026-05-06 09:30 show recovery/gap behavior with `430.1` open, `433.19` high, and `432.06` close; later same-day hold and 2026-05-07 action are unavailable. |
| 4 | `2026-05-06T15:30:00-04:00` | `ideal_recovery_hold_candidate` | Completed rows through 2026-05-06 show the recovery session ending at `430.6973` after an intraday `433.19` high; no 2026-05-07 follow-through is available. |
| 5 | `2026-05-07T10:30:00-04:00` | `ideal_follow_through_extension_candidate` | Completed rows through 2026-05-07 10:30 include follow-through to the `437.42` window high; later pullback and 2026-05-08 rows are unavailable. |
| 6 | `2026-05-08T15:30:00-04:00` | `ideal_pullback_hold_or_no_fresh_trigger_review` | Full selected window is available and ends after 2026-05-08 pullback/hold rows with `433.795` close; this row can review follow-through, stale/spent/no-fresh-trigger, or hold context only if the future fixture asserts it from replay evidence. |
...
- The fixture source row range is rows 204-238.
- The fixture source window is `2026-05-04T09:30:00-04:00` through `2026-05-08T15:30:00-04:00`.
- The source window row count is 35.
...
- Candidate lifecycle may progress from retest-low context to base-after-retest development, recovery confirmation candidate, recovery hold candidate, follow-through/extension candidate, and pullback-hold or no-fresh-trigger review.
...
- exact trigger level inside the source-backed `433.19` to `437.42` recovery/follow-through area
- exact invalidation level near the `413.2801` retest low zone if accepted
...
Create GLD Ideal 001 replay fixture JSON asset only, using this fixture specification. Do not create generated replay reports, chart outcomes, aggregate closeout, watcher work, option P&L, account sizing, production readiness claims, or live trade decisions in this specification-review task.
...
- GLD Ideal fixture JSON correctness
