"""Local-only Day 60 outcome diagnostics evaluation.

This module accepts the in-memory Day 60 outcome scoring summary only and
...
DAY60_OUTCOME_DIAGNOSTIC_FIX_PATHS = {
    ...
    "data_quality_or_missing_evidence": "collect or preserve missing source-backed outcome evidence",
    "market_context_review": "review available market-context evidence without inventing missing context",
    "outcome_scoring_review": "review outcome scoring labels, windows, and terminal evidence",
    "review_logging_review": "review rejected row logging, reasons, and review packet traceability",
    "user_facing_workflow_review": "review user-facing review workflow evidence and handoff clarity",
}

DAY60_OUTCOME_DIAGNOSTICS_RESULT_FIELDS = (
    "watch_only",
    "outcome_diagnostics_only",
...
_SUMMARY_REQUIRED_FIELDS = set(DAY60_OUTCOME_SCORING_SUMMARY_RESULT_FIELDS)
...
_FORBIDDEN_DIAGNOSTIC_FIELD_NAMES = frozenset(
    {
        "account",
        "account_size",
        "account_sizing",
...
_BUCKET_TO_CANDIDATE = {
    ...
    "inconclusive": "candidate: outcome scoring evidence may be incomplete or inconclusive",
    "partial_follow_through": "candidate: follow-through scoring may need more outcome evidence",
    "unavailable_evidence": "candidate: missing or unavailable evidence may limit scoring",
}


def evaluate_day60_outcome_diagnostics(
    outcome_summary: dict[str, Any],
) -> dict[str, Any]:
    """Return an in-memory Day 60 outcome diagnostics summary."""
    if type(outcome_summary) is not dict:
        raise TypeError("Day 60 outcome diagnostics outcome_summary must be a dict")

    _validate_outcome_summary(outcome_summary)

    diagnostic_findings = []
    unavailable_evidence = _collect_summary_unavailable_evidence(outcome_summary)

    for row in outcome_summary["accepted_rows"]:
        diagnostic_findings.extend(_diagnostic_findings_for_row(row))

    if outcome_summary["unavailable_outcome_fields"]:
        diagnostic_findings.append(
            _summary_unavailable_evidence_finding(
                outcome_summary["unavailable_outcome_fields"]
            )
        )

    if outcome_summary["rejected_rows"]:
        diagnostic_findings.append(
            _rejected_rows_finding(outcome_summary["rejected_rows"])
        )

    diagnostic_gap_counts = {category: 0 for category in DAY60_OUTCOME_DIAGNOSTIC_FAILURE_CATEGORIES}
    likely_cause_candidates = []
    next_fix_paths = {}
    for finding in diagnostic_findings:
        ...
        next_fix_paths[category] = DAY60_OUTCOME_DIAGNOSTIC_FIX_PATHS[category]

    return {
        "watch_only": True,
        "outcome_diagnostics_only": True,
        "optimization_started": False,
        "rows_processed": outcome_summary["rows_processed"],
        "rows_accepted": outcome_summary["rows_accepted"],
        "rows_rejected": outcome_summary["rows_rejected"],
        ...
        "rejected_rows": deepcopy(outcome_summary["rejected_rows"]),
...
def _validate_outcome_summary(outcome_summary: Mapping[str, Any]) -> None:
    _reject_forbidden_diagnostic_fields(outcome_summary, path=())

    missing_fields = [
        field_name
        for field_name in DAY60_OUTCOME_SCORING_SUMMARY_RESULT_FIELDS
        if field_name not in outcome_summary
    ]
    if missing_fields:
        raise ValueError(
            "Missing required Day 60 outcome scoring summary fields: "
            + ", ".join(missing_fields)
        )

    for field_name in _EXPECTED_TRUE_BOUNDARY_FIELDS:
        if outcome_summary[field_name] is not True:
            raise ValueError(
                f"Day 60 outcome diagnostics requires {field_name}=True"
            )
    for field_name in _EXPECTED_FALSE_BOUNDARY_FIELDS:
        if outcome_summary[field_name] is not False:
            raise ValueError(
                f"Day 60 outcome diagnostics requires {field_name}=False"
            )

    if type(outcome_summary["accepted_rows"]) is not list:
        raise TypeError("Day 60 outcome diagnostics accepted_rows must be a list")
    if type(outcome_summary["rejected_rows"]) is not list:
        raise TypeError("Day 60 outcome diagnostics rejected_rows must be a list")
    if type(outcome_summary["unavailable_outcome_fields"]) is not list:
        raise TypeError(
            "Day 60 outcome diagnostics unavailable_outcome_fields must be a list"
        )
    if type(outcome_summary["bucket_counts"]) is not dict:
        raise TypeError("Day 60 outcome diagnostics bucket_counts must be a dict")

    for index, row in enumerate(outcome_summary["accepted_rows"]):
...
def _validate_accepted_row_boundaries(row: Any, index: int) -> None:
    ...
    if row.get("watch_only") is not True:
        raise ValueError(
            f"Day 60 outcome diagnostics accepted_rows[{index}] must preserve "
            "watch_only=True"
        )
...
def _validate_no_trade_boundary(boundary: Any, index: int) -> None:
    if type(boundary) is not dict:
        raise TypeError(
            f"Day 60 outcome diagnostics accepted_rows[{index}].no_trade_boundary "
            "must be a dict"
        )
    for field_name in (
        "no_trade",
        ...
        if boundary.get(field_name) is not True:
            raise ValueError(
                f"Day 60 outcome diagnostics accepted_rows[{index}]."
...
def _validate_no_hindsight_boundary(boundary: Any, index: int) -> None:
    if type(boundary) is not dict:
        raise TypeError(
            f"Day 60 outcome diagnostics accepted_rows[{index}].no_hindsight_boundary "
            "must be a dict"
        )
    for field_name in (
...
def _diagnostic_findings_for_row(row: Any) -> list[dict[str, Any]]:
    ...
    if review_bucket == "strong_follow_through" or review_bucket == "blocked_correctly":
        return []
    ...
    if category is None:
        return [
            _build_finding(
                ...
                likely_cause_text=(
                    "candidate: unmapped outcome bucket may need scoring review"
                ),
            )
        ]
...
def _summary_unavailable_evidence_finding(
    unavailable_outcome_fields: list[Any],
) -> dict[str, Any]:
    return {
        "diagnostic_category": "data_quality_or_missing_evidence",
        "source_bucket": "unavailable_outcome_fields",
        "row_id": "SUMMARY",
        ...
        "evidence_used": [],
        "unavailable_evidence": deepcopy(unavailable_outcome_fields),
        "likely_cause_candidates": [
            {
                "label": "candidate",
                "candidate": "candidate: unavailable outcome fields may limit diagnostics",
                "evidence_basis": deepcopy(unavailable_outcome_fields),
            }
        ],
        "next_fix_path": DAY60_OUTCOME_DIAGNOSTIC_FIX_PATHS[
            "data_quality_or_missing_evidence"
        ],
    }
...
def _rejected_rows_finding(rejected_rows: list[Any]) -> dict[str, Any]:
    return {
        ...
        "next_fix_path": DAY60_OUTCOME_DIAGNOSTIC_FIX_PATHS["review_logging_review"],
    }
...
def _build_finding(
    *,
    ...
    return {
        ...
        "next_fix_path": DAY60_OUTCOME_DIAGNOSTIC_FIX_PATHS[category],
    }
...
def _collect_summary_unavailable_evidence(
    ...
    unavailable_evidence = deepcopy(outcome_summary["unavailable_outcome_fields"])
    for row in outcome_summary["accepted_rows"]:
        if type(row) is dict:
...
def _reject_forbidden_diagnostic_fields(value: Any, path: tuple[str, ...]) -> None:
    if isinstance(value, Mapping):
        for key, nested_value in value.items():
            ...
            if (
                normalized_key in _FORBIDDEN_DIAGNOSTIC_FIELD_NAMES
