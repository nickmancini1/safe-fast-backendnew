"""Local-only Day 60 optimization readiness gate.

This module accepts the in-memory Day 60 outcome diagnostics summary only and
returns an in-memory optimization readiness summary without side effects.
"""
...
_FORBIDDEN_OPTIMIZATION_READINESS_FIELD_NAMES = frozenset(
    {
        "account",
        "account_size",
        "account_sizing",
...
def evaluate_day60_optimization_readiness(
    outcome_diagnostics_summary: dict[str, Any],
) -> dict[str, Any]:
    """Return an in-memory Day 60 optimization readiness summary."""
    if type(outcome_diagnostics_summary) is not dict:
        raise TypeError(
            "Day 60 optimization readiness outcome_diagnostics_summary must be a dict"
        )

    _validate_outcome_diagnostics_summary(outcome_diagnostics_summary)
    ...
    for index, finding in enumerate(outcome_diagnostics_summary["diagnostic_findings"]):
            ...
            missing_evidence_items.append(
                {
                    "index": index,
                    "row_id": item["row_id"],
                    "reason": "evidence is missing without an explicit unavailable marker",
                }
            )
        if not item["ready"]:
    ...
    return {
        "watch_only": True,
        "optimization_readiness_only": True,
        "optimization_started": False,
        "ready_for_optimization": not blocked_items,
        "rows_processed": outcome_diagnostics_summary["rows_processed"],
        "diagnostic_findings_reviewed": len(
            outcome_diagnostics_summary["diagnostic_findings"]
        ),
...
def _validate_outcome_diagnostics_summary(summary: Mapping[str, Any]) -> None:
    _reject_forbidden_readiness_fields(summary, path=())

    missing_fields = [
        field_name
        for field_name in DAY60_OUTCOME_DIAGNOSTICS_RESULT_FIELDS
        if field_name not in summary
    ]
    if missing_fields:
        raise ValueError(
            "Missing required Day 60 outcome diagnostics fields: "
            + ", ".join(missing_fields)
        )
    ...
    for field_name in _EXPECTED_TRUE_BOUNDARY_FIELDS:
        if summary[field_name] is not True:
            raise ValueError(
                f"Day 60 optimization readiness requires {field_name}=True"
            )
    for field_name in _EXPECTED_FALSE_BOUNDARY_FIELDS:
        if summary[field_name] is not False:
            raise ValueError(
                f"Day 60 optimization readiness requires {field_name}=False"
            )

    if type(summary["diagnostic_findings"]) is not list:
        raise TypeError(
            "Day 60 optimization readiness diagnostic_findings must be a list"
        )
    if type(summary["unavailable_evidence"]) is not list:
        raise TypeError(
            "Day 60 optimization readiness unavailable_evidence must be a list"
        )

    for index, finding in enumerate(summary["diagnostic_findings"]):
        if type(finding) is not dict:
            raise TypeError(
                "Day 60 optimization readiness diagnostic_findings"
...
def _affected_system_area(finding: Mapping[str, Any]) -> str | None:
    ...
    if area not in DAY60_OPTIMIZATION_ALLOWED_SYSTEM_AREAS:
        return None
    return area
...
def _has_evidence_or_unavailable_marker(finding: Mapping[str, Any]) -> bool:
    ...
    unavailable_evidence = finding.get("unavailable_evidence")
    if _has_explicit_unavailable_marker(unavailable_evidence):
        return True
...
def _reject_shallow_label_without_evidence_or_fix_path(
    ...
    if _has_evidence_or_unavailable_marker(finding) and _non_empty_string_or_none(
        ...
        return
    raise ValueError(
        "Day 60 optimization readiness rejects shallow optimization labels "
...
def _reject_fact_language_in_likely_causes(
    ...
    for candidate in candidates:
        ...
        if label in {"fact", "root_cause", "confirmed"}:
            raise ValueError(
                "Day 60 optimization readiness requires likely causes to remain "
...
def _reject_forbidden_readiness_fields(value: Any, path: tuple[str, ...]) -> None:
    if isinstance(value, Mapping):
        for key, nested_value in value.items():
            ...
            if (
                normalized_key in _FORBIDDEN_OPTIMIZATION_READINESS_FIELD_NAMES
